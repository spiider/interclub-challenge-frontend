              ...
              {this.state.searchValue.length > 0 && this.props.data.length > 0 &&
                ...
                <SuggestedList>
                  {this.props.data.map((user, index) => index === this.state.highLighted ?
                    <SuggestionHighlight key={user.id} onClick={this.handleSelectClick.bind(this, user)} >{getHighlightedText(user.name, searchValue)}<UserNumber># {user.number}</UserNumber> </SuggestionHighlight> :
                    <SuggestionLine key={user.id} onClick={this.handleSelectClick.bind(this,  user)} >{getHighlightedText(user.name, searchValue)}<UserNumber># {user.number}</UserNumber></SuggestionLine>)}
